...
import { cryptoService, useAuthRepository } from '@nao-todo/infrastructure'
import { getRequesterImpl } from '@nao-todo/shared'

/**
 * 认证用例（桌面版保持远程后端认证，并联动本地数据解锁）
 * @description signIn 成功后用登录密码确保本地密钥包就绪（首次 setup / 之后 unlock）；
 *              signOut 成功后清空内存密钥（本地密文不可读）。
 * @param store 认证状态存储
 * @returns 认证用例
 */
export const useAuthUseCase = (store: AuthStore) => {
    ...
    useCase.signIn = async (signInViewObject: SignInViewObject) => {
        ...
        try {
            await cryptoService.ensureUnlocked(signInViewObject.password)
        } catch (unlockErr) {
