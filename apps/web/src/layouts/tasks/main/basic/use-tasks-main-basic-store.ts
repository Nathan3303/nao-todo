...
const useTasksMainBasicStore = defineStore('TasksMainBasicStore', () => {
    // Router & Stores
    ...
    watch(
        ...
        async ([newReadyState, newPage, newGetTodosOptions]) => {
            // 判断 newReadyState 是否为 3
            if (newReadyState !== 3) return
            // 判断 viewProps 是否存在
            if (!viewProps.value) return
            // 判断 viewProps.category 是否是 'basic'
            if (viewProps.value.category !== 'basic') return
            // 判断 viewProps.id 是否相同
            // if (iBasicId.value === viewProps.value.id) return
            // 重置加载状态
            loading.value = true
            error.value = ''
            // 调用 API 请求数据
            ...
            // 记录已请求的清单 Id，避免重复请求
            // iBasicId.value = viewProps.value.id
        },
    ...
    const showTodoDetails = async (id: string) => {
        await router.push({ name: 'tasks-basic', params: { todoId: id as string } })
    }
...
export default useTasksMainBasicStore
