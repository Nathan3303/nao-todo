...
import { SEARCH_VIEW_CONTEXT_KEY } from './context'
...
// @composable 搜索页共享状态（单一真源：由 useSearchView 实例化，侧栏注入同一实例）
const {
    history,
...
// @method 复现常用搜索（完整条件应用 + URL 同步由引擎 watch 负责；回焦由侧栏负责）
const applySavedSearch = (item: SavedSearch) => {
    applyQuery(item.query)
}

// @provide 搜索视图上下文（侧栏与主区共享同一份状态 + 跨区回调）
provide(SEARCH_VIEW_CONTEXT_KEY, {
    savedSearches,
    removeSavedSearch: removeSavedSearchItem,
    renameSavedSearch: renameSavedSearchItem,
