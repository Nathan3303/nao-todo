<template>
    <nue-container class="todo-comment-details">
        <nue-header height="auto">
            <nue-div style="padding: 8px">
                <nue-text size="14px" weight="500">评论</nue-text>
                <nue-text color="gray" size="14px">{{ comments.length }}</nue-text>
            </nue-div>
        </nue-header>
        <nue-main>
...
const handleEditComment = async (id: Comment['id'], newContent: Comment['content']) => {
    // console.log('[TodoCommentDetails/handleEditComment]', id, newContent)
