...
- **验证分工（2026-09-25 用户定）**：**代码逻辑测试由舰队角色负责**（单元/集成/探针/变异/失败演练——即「可自动化、可复现、可给数字」的一切）；**视觉 / UI / 真机走查由用户本人执行**（含打包后的桌面端观感、交互手感、Toast 观感、多端外观一致性等）⇒ **PM 不派「人工烟测 / 人工走查」类任务**（`T184` 时期的此类建议作废）；交接时 PM 须给出**可勾选的视觉验证清单**（前置：服务端/客户端部署到位），由用户勾选回报。
