...
export class UserUseCase {
    // 构造函数
    constructor(
        // private readonly userService: UserService, // 用户域
    ...
    /**
     * 更新用户配置
     * @param updatedVO 更新用户配置视图对象
     * @returns 更新结果
     */
    async updateUserConfig(updatedVO: UpdateUserConfigViewObject): GoAsync<void> {
        const [userConfigEntity, getError] = await this.userConfigRepo.get()
        if (getError !== null) {
            return getError
        }
        const changeError = userConfigEntity.changeAppearence(updatedVO.appearance || '')
        if (changeError !== null) {
            return changeError
        }
        const saveError = await this.userConfigRepo.save(userConfigEntity)
        if (saveError !== null) {
            return saveError
        }
        this.userStore.updateUserConfig(updatedVO)
        return null
    }
