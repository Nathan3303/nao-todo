...
import { ProjectPreferenceRepository, ProjectRepository } from '@nao-todo/domain/project/repositories'
import {
...
export class ProjectUseCase {
    ...
    async loadProjectPreference(projectId: ProjectViewObject['id']): GoAsync<void> {
        // 调用领域层方法
        const [preferenceEntity, err] = await this.projectPreferenceRepo.get(projectId)
        if (err !== null) return err
        // 转换为视图对象
