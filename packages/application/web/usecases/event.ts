...
export interface EventStore {
    ...
    updateEvents: (newEvents: EventViewObject[]) => void
...
export class EventUseCase {
    ...
    async resortWithRebuild(originalId: string, boundId: string, isBefore: boolean): GoAsync<void> {
        ...
        // 乐观更新本地
        this.store.setEvents(eventsToUpdate)
        ...
        // 调用后端批量更新
        const [batchResult, err] = await this.eventDomain.batchUpdate(eventEntities as any)
        if (err !== null) return err
