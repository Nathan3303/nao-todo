...
import type { TaskListEmits, TaskListMultiSelectPayload, TaskListProps } from './types'

export default (props: TaskListProps, emit: TaskListEmits) => {
    // @state 多行选择范围记录 - 用于多行选择
    const selectRange = reactive<TaskListMultiSelectPayload['selectRange']>({
        start: -1,
        end: -1,
        original: -1
    })
