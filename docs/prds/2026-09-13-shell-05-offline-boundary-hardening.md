...
## 14. 待决项（用户 2026-09-13：先验收本轮，SHELL-06 与 Web 统一架构暂缓）

| # | 待决 | 状态 |
| :--- | :--- | :--- |
| P1 | SHELL-06 是否立项（A1）还是先只保证本地可用（A2） | 待用户 |
| P2 | Web 端"统一架构"含义（B1 本地优先 / B2 仅统一装配 / B3 全量对齐）+ 动机 | 待用户；建议先 arch 可行性 ADR |
| P3 | D11–D15（拆分/`nextAttemptAt`/周期退避/阈值文案/G12·G14 随单） | 待用户 |

> 本轮（SHELL-05）仅验收"离线可进入 + 本地数据可用 + 回传缺口已定位"，不含 SHELL-06 实现。
