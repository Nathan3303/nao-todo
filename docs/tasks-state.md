...
- **需求池**：
    ...
    - **DEF-2**（舰队资产缺陷，**入池待用户决定**）：`.agents/scripts/nao-fleet.sh` 行 577 默认值行漏初始化 `TASK=""`，而 `set -u` 下 `cmd_ensure "$FORCE" "$MODEL" "$TASK" ...`（行 597）在 **`ensure` 不带 `--task`** 时抛「TASK: 未绑定的变量」⇒ **常驻会话拉起路径整体不可用**（派生会话路径因显式传 `--task` 不受影响）。**PM 越过方式**：`TASK= bash .agents/scripts/nao-fleet.sh ensure <role>`（未改脚本，守零代码红线）。修法为 1 行（`TASK=""`），但 `.agents/**` 属共享舰队资产 ⇒ **待用户授权后再改**。
- **更新**：2026-09-23
...
| :------- | :------- | :--- | :-------------------------------------------------------------------------- | :--------- | :------ |
| T94      | qa       | 测试 | AC1–AC5 用例先行（三渲染器纯函数 + 对话框状态机 + footer 流程反转；预期红） | 2026-09-23 | AC1–AC5 |
