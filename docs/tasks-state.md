...
| :------- | :------- | :--- | :------------------------------------------------------------------------- | :------------------------------ |
| T96      | rd-fe    | 前端 | 流程反转 + `use-export-task` 状态机 + 对话框三态/格式分段/重试/iframe 预览 | 等 T95 回执（同包单写者，串行） |
...
| :------- | :------- | :--- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------- | :-------------- |
| T95      | rd-fe    | 前端 | 三纯函数渲染器（`generateTaskJson` / `generateTaskHtml` 含 §14 视觉规格）+ `ExportTaskNode` additive 扩展 + i18n。**在制品裁决：D10（合计段 `x=0 且 y>0` 仍输出 `0/y`，判 qa 用例为过度断言）+ 提交时序已定【qa 先提交断言修正，rd-fe 后才提交】** | 2026-09-23 | AC1 / AC3 / AC4 |
