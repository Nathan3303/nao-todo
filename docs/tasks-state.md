...
- **进行中批次**：**无** —— TASK-13/15/16/17/18/19/19B/20 **全部 ✅ 用户终签通过（2026-09-22）**，见 §二。
...
- **CodeGraph 抽检固化（建议）**：ADR **C12** 要求「改动覆盖层/`z-index` 后做真实浏览器 `elementFromPoint` 抽检」，目前仅存在于验收清单的手工片段；建议固化为可复用脚本/片段（需实现方提交，可另立单）。
