...
- **进行中批次**：**TASK-21（收尾批次，用户 2026-09-22 批准）** —— T92 把 ADR **C12 命中抽检固化**进 `scripts/electron-smoke/`（**零新依赖**）+ T93 清 TASK-16 登记的零引用死代码（`.cal-nav-btn` ×3 / `.cal-aside-toggle` ×1，我已核实零引用）。_（TASK-13/15/16/17/18/19/19B/20 已全部 ✅ 用户终签通过，见 §二。）_
...
| :------- | :------- | :--- | :------------------------------------------------------------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------ |
| T92      | rd-fe    | 前端 | 命中抽检固化进 `scripts/electron-smoke/`（ADR C12；零新依赖） | ✅ 已回执并（PM）验收 `149305fe`（3 文件；PM 复跑 993 例全绿 / `vp check` exit 0；新模块 621 行，含 **SKIP 不假绿** + **`stack`/`blank`/`overlay` 判据**）—— ⏳ **端到端未实跑**（需 `NAO_QA_EMAIL/PASSWORD`） |
| T93      | rd-fe    | 前端 | 清零引用死代码（`.cal-nav-btn` ×3 / `.cal-aside-toggle`）     | ✅ 已回执并（PM）验收 `b32e05a6`（1 文件；删恰 4 条零引用规则 + 替换原「本单不清理」悬空注释；993 例全绿、月视图子集 187 例全绿）                                                                              |
