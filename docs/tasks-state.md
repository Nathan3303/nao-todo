...
| :------- | :------- | :--- | :------------------------------------------------------------ | :--------- | :------ |
| T92      | rd-fe    | 前端 | 命中抽检固化进 `scripts/electron-smoke/`（ADR C12；零新依赖） | 2026-09-22 | C12     |
| T93      | rd-fe    | 前端 | 清零引用死代码（`.cal-nav-btn` ×3 / `.cal-aside-toggle`）     | 2026-09-22 | —       |
...
- **命中抽检固化（进行中 → T92）**：ADR **C12** 要求「改动覆盖层/`z-index` 后做真实浏览器 `elementFromPoint` 抽检」；原仅存在于验收清单的手工片段 ⇒ 已派 **T92** 固化进既有 `scripts/electron-smoke/`（CDP 真实渲染、零第三方依赖）。
