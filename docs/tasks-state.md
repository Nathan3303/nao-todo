...
| :------- | :------- | :--- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------- |
| T97      | qa       | 测试 | 独立验收（AC1–AC5 + 门禁 + 移动端红线 + Markdown 源码级零变更（非仅复跑）+ **M1–M5 变异验证（判别力）** + 4 处 supersede 等强度论证 + `env.d.ts` 必要性独立复核） | 等 T96 回执 |

### 进行中

| 任务编号 | 目标会话 | 角色 | 概要                                                                                                                                                                                                                                                                                                                                                                                | 派发时间   | 对应 AC         |
| :------- | :------- | :--- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------- | :-------------- |
| T96      | rd-fe    | 前端 | 流程反转 + `use-export-task` 状态机（loading/error/ready + 重试防重入 + 一次取数缓存）+ 对话框三态/格式分段/iframe `srcdoc` 预览。**红窗口闭合点**：完成后 `vp check` 须 0 错（当前 27）+ 目录 23 红→ 0；**本提交禁止 `--no-verify`**。**在制品裁决：D12（D8 清单追加第 4 处 supersede `export-regression-change1.test.ts:306` button 计数）+ 提交时序【qa 先修，rd-fe 后才提交】** | 2026-09-23 | AC1 / AC2 / AC5 |
