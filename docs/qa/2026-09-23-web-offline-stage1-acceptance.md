...
> ⚠️ **本轮复核更新（T111b，2026-09-23）**：NEW-1（= DEF-21）、NEW-2（= DEF-22）**均已修复且经我独立复核通过**，**AC8 数据面已改判为通过**；NEW-3 登记为 DEF-23（arch 挂账）。**§一–§十一 首轮结论保留作历史基线**，最新结论以 **§十二 本轮复核** 为准。
...
| 2026-09-23 | **追加 §十二 本轮复核（T111b）**：DEF-21 探针首/本轮对照（AC8 改判通过）+ DEF-22 复核 + 门禁复跑 + 变异复验（M1–M6 + DEF-21 新分支）+ DEF-25 评估 + 最终发布建议 |

---

## 十二、本轮复核（T111b）—— DEF-21 / DEF-22 复核 + AC8 改判 + 最终结论

> **性质**：T111 首轮结论（§一–§十一）**保留作历史基线，不重写**；本节为本轮**独立复核**（基线 HEAD `5bcfad4d` = `7b9cb500` + 1 个 **docs-only** `tasks-state.md`；工作区 0）。
> **复核对象**：`DEF-21` 修复 `9f0228ba`（AC8 数据面）+ `DEF-22` 修复 `7b9cb500`（明文用户可见声明）——**不采信 worker 自述，逐项独立验证**。

### 12.1 DEF-21（AC8 数据面）—— ✅ 修复有效，**AC8 改判为通过**

**（1）首轮失败探针重跑对照（同格式输出）**

探针 = 首轮同一份（真实 `TaskRepoImpl` 远端 + 真实本地镜像 + **归一化 `ERR_NETWORK`（resolve 非 reject）** + jsdom + 真实 JWT 键）：

| 轮次                             | `PROBE list result` 输出                                                                                                            | `err`                          | 判定        |
| :------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------- | :----------------------------- | :---------- |
| **首轮（T111，修复前）**         | `null`                                                                                                                              | `网络错误，请检查您的网络连接` | ❌ 未回退   |
| **本轮（T111b，`9f0228ba` 后）** | `{"taskEntities":[{"id":"471981006071652352",…,"name":"镜像中的任务",…}],"pagination":{"total":1,"page":1,"limit":20,"maxPage":1}}` | `null`                         | ✅ 回退镜像 |

⇒ **同输入同探针下由「网络错误」变为「返回镜像数据、`err=null`」** ⇒ AC8「离线可看数据」数据面**改判为通过**。（探针脚本存 `/tmp/qa-t111/qa-probe-fallback.test.ts`，定向运行后已删，未入仓。）

**（2）修复读码核验**（不采信转述）：

- 新增 `fallback/network-failure.ts`：`NORMALIZED_NETWORK_ERROR_MESSAGES` **全等**匹配 requester 归一化三文案（`ERR_NETWORK→50300` / `ECONNABORTED→40800` / `TOO_MANY_REQUESTS→42900`），每项标注出处（`axios.ts` / `lynx.ts`）。
- `mirror-fallback.ts`：**双层判据** —— ① `catch`（抛出型：凭证类上抛、其余回退）② `[null, message]` 归一化元组型（网络文案 ⇒ 回退；业务/凭证文案 ⇒ 原样返回）；写方法与未列入读方法仍原样透传（C-59）。

**（3）独立运行生产形态测试**：`packages/infrastructure/src/persistence-go/fallback/__tests__/mirror-fallback.test.ts` **13 例全绿**，含 `it.each` 三码 × 真实 `TaskRepoImpl` + 业务失败元组不回退 + 凭证 10041 元组不回退 + **真实 `AxiosRequester` 端到端**（真实拦截器产物 ⇒ 判据命中 ⇒ 回退镜像）。

**（4）变异（独立重做）**：把新分支 `if (isNormalizedNetworkErrorTuple(result))` 改为 `if (false && …)` ⇒ 该文件 **4 failed | 9 passed (13)** ⇒ 与 `T112` 自述「4 红/9 绿」**一致**；已还原。

### 12.2 DEF-22（明文用户可见声明）—— ✅ 实际存在且内容正确

| 核验点              | 结论                                                                                                                                                                                                        |
| :------------------ | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 设置页声明实际存在  | ✅ `apps/web/src/components/settings/local-data-notice/index.vue`，经 `settings/app-setter/index.vue` 挂载                                                                                                  |
| 覆盖 RS-1/RS-3/RS-4 | ✅ 五项：明文保存 · **设备即凭据**（RS-1） · **访问边界**（桌面同 profile 本地页 / web origin，RS-3） · **浏览器可能清除**（Safari 7 天，RS-4） · **两端差异**                                              |
| **无「加密」字样**  | ✅ 源码 `grep`/`sed` 实测无；测试断言 `not.toContain('加密'/'已加密'/'受保护')`                                                                                                                             |
| 首启告知实际存在    | ✅ `plaintext-notice/plaintext-notice-banner.vue`，经 `views/index/index.vue` 挂载（主界面）；**非模态**（`div` + 关闭按钮，不拦截交互）                                                                    |
| 关闭后不再展示      | ✅ `plaintext-notice.test.ts`：「点击关闭 ⇒ 写 `nao.plaintextNoticeAck=1` 且横幅消失」「已读 ⇒ 不再渲染」                                                                                                   |
| **登出后不重复**    | ✅ **读码 + 实测双证**：键单一真源 `packages/shared/constants/storage-keys.ts::PLAINTEXT_NOTICE_ACK_KEY`；已入 `DEVICE_LEVEL_STORAGE_KEYS` 白名单；`deletion-wipe.test.ts` 新增例断言「登出清库后标记保留」 |
| 三处 locales 齐备   | ✅ `types.ts` / `zh-CN.ts` / `en-US.ts`（`notice.plaintext.*` 3 + `settings.localData.*` 7）                                                                                                                |
| 独立运行            | ✅ `plaintext-notice.test.ts` 3 例 + `local-data-notice.test.ts` 2 例 + `deletion-wipe.test.ts` 7 例，全绿                                                                                                  |

### 12.3 全范围门禁独立复跑（HEAD `5bcfad4d`）

| #   | 门禁                                   | 本轮结果                                                                                                      | 与 PM 基线                |
| :-- | :------------------------------------- | :------------------------------------------------------------------------------------------------------------ | :------------------------ |
| ①   | `pnpm exec vp check`                   | **rc=0**；`All 1391 files are correctly formatted` + `no warnings, lint errors, or type errors in 1192 files` | ✅ 一致（1391 / 1192）    |
| ②   | `pnpm exec vp test --run`（**全仓**）  | **rc=0**；**150 文件 passed / 1250 例 passed / 红 0**（Duration 104.87s）                                     | ✅ 一致（150 / 1250 / 0） |
| ③   | `pnpm run guard:ddd`                   | **rc=0**（`[guard:ddd] OK`）                                                                                  | ✅ 一致                   |
| ④   | `pnpm --filter @nao-todo/webapp build` | **rc=0**                                                                                                      | ✅ 一致                   |
| ⑤   | `pnpm run desktop:build`               | **rc=0**                                                                                                      | ✅ 一致                   |
| ⑥   | 移动端红线 diff                        | **0**                                                                                                         | ✅ 一致                   |

### 12.4 变异复验（改动后；均已还原，工作区最终 0）

| #          | 变异落点                                                | 本轮实测                                                    | 结论  |
| :--------- | :------------------------------------------------------ | :---------------------------------------------------------- | :---- |
| **DEF-21** | `mirror-fallback.ts` 禁用新增网络类分支（`false && …`） | `mirror-fallback.test.ts` **4 failed / 9 passed**           | ✅ 红 |
| M1         | `endRun` 的 `pullExecuted` 由 `ok` 反推                 | `sync.test.ts` **2 failed / 32 passed**                     | ✅ 红 |
| M2         | 只读闸门套到 desktop 接线                               | `write-gate-wiring.test.ts` **1 failed / 2 passed**         | ✅ 红 |
| M3         | 触顶 N 换固定 `2000`                                    | `use-mirror-loaded-count.test.ts` **2 failed / 2 passed**   | ✅ 红 |
| M4         | `requireCurrentUserId` 回退 `?? ''`                     | `userId-hard-fail.test.ts` **4 failed / 1 passed**          | ✅ 红 |
| M5         | **镜像新鲜度不落盘**（回退路径改动后重点复验）          | `mirror-status-persistence.test.ts` **2 failed / 2 passed** | ✅ 红 |
| M6         | 清库纳入 `deletionSchedules`                            | `deletion-wipe.test.ts` **1 failed / 6 passed**             | ✅ 红 |

> M1–M6 在 DEF-21 改动后**仍全部如期转红**（M6 由 5 绿 → 6 绿系 T113 新增 1 例，非回归）。

### 12.5 DEF-25 评估（P3，挂账）—— **同意 PM「不强制返工」**

- **事实修正**：判据在**仓储边界**只能拿到 `message` 字符串（归一化顶层 `code` 在 Go 仓储返回元组时已丢失）⇒ 现实现是**全等匹配文案**，并非「同时认 code」；PM 描述中的「以 code 为准」在不改 Go 仓储返回形态的前提下**不可直接实现**。
- **接受理由**：新增的**真实 `AxiosRequester` 端到端**用例把「判据 ↔ requester 实际产物」钉死 —— 文案漂移即转红，**DEF-21 复发路径已被守护**。
- **可选低成本加固（非阻塞，供后续）**：把 requester 的三条归一化文案抽为 `packages/shared/requester` 导出常量，由 `network-failure.ts` 引用，消除字面量二次真源（比「改成按 code 判定」改动小得多）。

### 12.6 本轮未发现新问题

- 无新增缺陷。`DEF-23`（ADR 门退役口径）/ `DEF-24`（测试卫生）为 T111 首轮登记项，本轮确认已入池、按 PM 判定挂账，**不在本轮复核范围**。
- 首轮「JSON.stringify 不含 `u-1`」等禁 PII 断言、AC18 两端启动日志等**未受影响**（全仓 1250 例全绿）。

### 12.7 最终 AC 结论（复核后）

- ✅ **AC8 改判为通过**（文案三态 + 冷启动落盘 + **数据面回退镜像实测生效**）⇒ **AC 汇总：✅ 22 条 ｜ 👁 1 条（AC17）**
- AC17 人眼项中的「设置页明文声明」**已可核验（DEF-22 落地）**；其余人眼项（冷启动观感 / 文案可读性 / 离线标识视觉 / web CSP 实站 / Z20 真机耗时 / 多标签视觉跳转）仍待用户终签。

### 12.8 最终发布建议

**建议：Go（具备用户终签条件）** —— 首轮唯一未过项（DEF-21，AC8 数据面）经独立复核确认修复有效，DEF-22 用户可见声明落地；**全范围门禁与变异全部通过**，无新增未过项。

1. 剩余仅 **人眼项（AC17）+ 部署侧项（web CSP 实站 / Z20 真机耗时）** 属用户/ops 终签范围。
2. `DEF-23` / `DEF-24` / `DEF-25` 挂账项**不阻塞**阶段一发布（DEF-25 有 e2e 守护）。
3. 姿态不可逆（D-5）⇒ **阶段一单独发版**、灰度后观察（沿用既有策略）。