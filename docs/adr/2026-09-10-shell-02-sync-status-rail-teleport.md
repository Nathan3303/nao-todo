...
| **r5** | **2026-09-14** | **v1.7.2 用户重构废除 C9 内容门控**（`33f6067a`）：面板内容（信息行 + footer 按钮）改为库**常驻挂载**（直挂面板根 `<ul>`），打开/关闭仅翻转 `data-visible`，组件不再在 `@close` 卸载内容。**C9 语义修订**（“@open 渲染 / @close 移除” → “内容常驻、可见性由库 data-visible 控制”）；**C10 断言口径修订**（关闭态只断言 `data-visible=false`，不再断言“门控内容不存在”）；单测同步改写；R6 风险消解 |
...
| #       | 约束（硬）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | 依据                                                                                           |
| :------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :--------------------------------------------------------------------------------------------- |
| **C9**  | **（r5 修订，v1.7.2 `33f6067a`）** 面板**内容**由库**常驻挂载**（信息行 + footer 按钮直挂面板根 `<ul>`），打开/关闭仅翻转 `data-visible`（`true`/`false`），组件**不再**在 `@open`/`@close` 卸载内容；`afterClose` 不依赖（jsdom 无动画不触发，内容本就常驻）。原“`@open` 渲染 / `@close` 移除”门控（`v-if="panelOpen"`）已废除                                                                                                                                                                                                       | r2 jsdom 探针（门控时期）；r5 实测（v1.7.2 后 jsdom 关闭态内容仍常驻、仅 `data-visible` 翻转） |
| **C10** | 单测“关闭态”断言口径：**只断言 `[data-visible="false"]`**（面板已关闭），**禁止**断言“池为空/面板 DOM 彻底移除”（jsdom 下不成立 → 假失败）；**r5 修订**：内容常驻后也不得断言“门控内容不存在”（内容不再由组件卸载，关闭态结构完整为预期）                                                                                                                                                                                                                                                                                             | r2 jsdom 探针；r5（v1.7.2 后内容常驻为设计契约）                                               |
...
| R6                                   | jsdom 无动画 → `afterClose` 不触发                                                                                                                                                                                                                 | 关闭态“内容残留” → 测试假失败                                                                                                                                 | ~~C9/C10~~（r5：v1.7.2 后内容常驻为设计契约，无“残留假失败”）                                                                                       |
...
- [ ] C9（r5 修订）面板内容常驻挂载、可见性由库 `data-visible` 控制（原 `@open`/`@close` 门控已废除）
- [ ] C10 关闭态断言口径（`data-visible=false`；r5 后不再断言“内容不存在”）
...
6. **r6（2026-09-23，T115）互记**：本组件的**物理落点迁入 webapp**（`apps/web/src/components/sync/sync-status-bar.vue` + `apps/web/src/hooks/use-{sync-status,manual-sync}.ts`），两端经既有装配缝引用 ⇒ **D-1 落点描述 / §4 证据路径 / §6 AC-13「web 零可见变化」失效 / §7 R8·R9** 由 `docs/adr/2026-09-23-two-end-sync-status-unification.md` 修订；**C1–C16 行为约束全部继续有效**；**r7（T115c 用户裁定）追加**：① 顶部内容区零挂载（`index.vue` 移除 `<offline-status />`）+ ②③④⑤ 全部迁入本组件面板；② **C14/D2 补注**：新增「数据可信度」走**正交角标通道**（`::after` 圆点；不完整·触顶=alert / 离线镜像=warn），**颜色通道与优先级一字不改**；③ **C5/AC-08 待裁定**（DP-6：抽屉分支是否也绑定 `railBottomHost`，否则 ≤445px 完全无同步状态）；④ §5 skill 偏离补注：迁入面板的信息行同样必须为 `<li>`；⑤ **C6′ 白名单扩展**（r9）：`NueConfirm` / `NueMessage` **追加**进「两版行为等价根导出」——证据 = 两版 `dist/types/components/{confirm,message}/*.d.ts` **逐字一致** + 实现均经 `mountPopupAnchor()`（弹层池）+ 均支持 `unuseCancelButton`；命名纪律：**`NueMessage.warn`**（不存在 `NueMessage.warning`）。两篇冲突以更晚落盘者为准。