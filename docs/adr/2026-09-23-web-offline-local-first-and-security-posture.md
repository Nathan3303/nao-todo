...
| K-5 | `shell-06` **C-44**（协议冻结 + 记录字段纯追加）                  | 明文/迁移标记若落 `meta` 表（非索引字段）**不违反**；**若需新增 Dexie 索引或 version bump ⇒ 必须回评审**。**⚠️ r13 邻接指针（2026-09-24，T161-r4）**：阶段二 2B OCC 的 **additive 协议面**（push `baseUpdatedAt` + `outcome: stale` + 偏好推送回传 `updatedAt`；**不改共享 create DTO、不新增错误码、HTTP 200 + 逐条 outcome**）**已按本条经 C-44 评审放行**（阶段二 ADR §9.1）；base 落**业务行非索引字段** ⇒ **不涉 Dexie 索引/version bump**。 |
...
| r13        | 2026-09-24                                                                                                                                                                                                                                                                            | **T161-r4（arch，doc-only）—— S17 邻接指针落地**：§2.3-K-5（SHELL-06 **C-44** 邻接）加指针 —— 阶段二 2B OCC 的 **additive 协议面**（push `baseUpdatedAt` + `outcome: stale` + 偏好推送回传 `updatedAt`；不改共享 create DTO、不新增错误码、HTTP 200 + 逐条 outcome）**已按 C-44 评审放行**（阶段二 ADR §9.1）；base 落业务行非索引字段 ⇒ **不涉 Dexie 索引/version bump**。**⛔ 未改代码**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | arch |
