...
| **r8** | **2026-09-22** | **C12 抽检已固化（T92）+ 回归判据细化**：落点 = 既有 `scripts/electron-smoke/`（CDP 真实渲染、**零第三方依赖**）新增 `checks/day-view-hit.mjs`（feature `day-view`，`--only day-view`）。**⚠️ 判别力关键结论（必须照此判红）**：**单看「条中点命中 `.cal-item`」没有判别力** —— 条内文本 `.cal-item-text` 带 `z-index: 3` 仍在命中栈顶 ⇒ 覆盖层回归时**该断言仍 PASS**（T92 实测：把 `.day-axis-track` 改回 `pointer-events: auto`，`mid` 仍绿而 `stack`/`blank`/`overlay.track` 三条同时转红）。**故回归判据 = ① `elementsFromPoint` 命中栈不含任何覆盖层（`stack`）② 空白点命中非条层且 `inTrack=false`（`blank`）③ 覆盖层 computed `pointer-events === 'none'`（`overlay.*`）**。**几何口径修正**：条左缘归属**左手柄**（手柄盒 = `[缘−3, 缘+5]`）⇒ 「条体」判据须用**缘+10px**（原 PM 派单写「缘+2 命中条本身」**错误，已由实测证伪**）。                                                                                                                                                                                                                                                                                                                                                                          |
...
- 名称：`position: sticky; left: 0` + `width: max-content; max-width: 100%`（**必须收缩到内容宽**，否则无可滑动余量）。 **（r5：`left` 由 `0` 调整为 `12px`；「全天」标签同口径 12px）**
...
- **结构契约（r5 放宽）**：`.day-allday-lane` 与 `.day-grid` **均为 `.day-scroll` 直接子节点、且泳道在网格之前**；**允许二者之间存在 `<nue-divider />`**（用户 2026-09-22 裁决，原「`nextElementSibling === .day-grid`」作废）。**禁**在 `.day-scroll` 与泳道之间、或泳道与网格之间插入**其它包裹层**。
...
- **抽检口径（r7 人工；r8 起已工具化）**：`bar-mid` 与 `bar-left` 应命中 `.cal-item`；`handle-r`/`handle-l` 命中手柄；`blank` 命中空白层。
    - **r8 补充（工具化落点）**：`scripts/electron-smoke/checks/day-view-hit.mjs`（feature `day-view`）已实现上表 + `allday` / `ticks` / `zoom`（×1 与 ×4 两档）各组；运行：`node scripts/electron-smoke/run.mjs --launch --feature day-view`（需注入 `NAO_QA_EMAIL` / `NAO_QA_PASSWORD`）。**无数据 ⇒ SKIP（绝不 PASS）**。
    - **r8 判红口径**：以 `stack` + `blank` + `overlay.*` 为准，**不要只看条中点命中**（见修订记录 r8：`mid` 无判别力）。
    - **r8 几何口径**：条左缘 `缘+2px` ⇒ **左手柄**；条体判据 ⇒ **缘+10px**。
