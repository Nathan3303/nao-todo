...
| **r3** | **2026-09-13** | **触发点补充（T1 探针实证，PM 裁定选项 a）**：sync push upsert 覆盖分支（`created=false`）写新 `project_id`/`parent_task_id` 却不发 E5/E6 ⇒ 计数漂移；**复用既有 E5/E6 补齐**（不新增事件），补边界 **B12**（含墓碑复活不重复发、`deleted_at` 覆盖分支为当前不可达的登记缺口）。无决策/选型变更 |
...
| E5  | `TaskMoved`             | `UpdateTask` **projectId 变更**；**sync push upsert 覆盖分支（`created=false`，B12）**                            | 旧项目 `-1`、新项目 `+1` + bump 两行                                                                                                                                                                                                                                                                                                                                           |
| E6  | `TaskParentChanged`     | `UpdateTask` parentTaskId 变更（A→B 或 A→0）；**sync push upsert 覆盖分支（`created=false`，B12）**               | 父 A `-1`、父 B `+1`                                                                                                                                                                                                                                                                                                                                                           |
...
- **B12「upsert 覆盖分支同样是 E5/E6 触发点」（r3 补充）**：sync push 的 tasks 行经 `CreateTask` → `repoImpl.Upsert`；`created=false` 的覆盖分支会写入新的 `project_id`/`parent_task_id`（`CreateTaskVOToUpdateMap`）却原样 `return nil`，**只发 E4/E3 的 `created=true` 分支不覆盖此路径 ⇒ T 由 A→B 推送后 A 仍为 1、B 仍为 0**。约束：
    - 覆盖分支须取旧值（`Upsert` 已加载 `existing`，旧值可得，实现上把旧值/变更标志带出即可，对应 B4 的同类要求），**仅实际变化时**发 E5/E6（同 `UpdateTask` 语义：E5 用 Old/NewProjectId；E6 旧父 `-1`、新父 `+1`，A→0 只 `-1`）。
    - **墓碑复活不得重复发 move/换父**：`Upsert` 对已软删记录返回 `created=true` ⇒ 走既有 E4 `+1`/E3 `+1` 分支（删除时已 `-1`，复活即重新计入）；此时**不得**再按旧项目 `-1`，否则双重扣减。
    - 同事务由 `CreateTask` 的 `txManager.Do` 覆盖（嵌套保护），push 每条独立事务（B10 不破）。
    - **登记缺口（当前不可达，不扩本单）**：同一覆盖分支还可写 `deleted_at`（服务端 `CreateTaskReq.DeletedAt` 接受墓碑），若经 tasks 行数组推墓碑则 `created=false` 不触发 E4 `-1`/E3 `-1`，属同类漂移。现状桌面端删除经 `syncTracker` `action='delete'` 进 `deletions` 数组（`sync-service.ts:600`），upsert 路径不携带 `deletedAt` ⇒ 不可达；若未来改为行内墓碑推送，须同步补 E4/E3 反减。
        - **可达时的实际后果比「漏减」更重（r3 复核）**：已墓碑行被再次覆盖（`Upsert` 返回 `created=existing.DeletedAt.Valid=true`）⇒ 走进 `created=true` 分支，**发出 E4/E3 的 `+1`（虚假增量）**，而行仍为已删除（口径 e 应为出局）。防护判据（实现由 RD 定）：本次覆盖仍写入墓碑时跳过 `+1`，即 `createTaskValueObject.DeletedAt.ShouldUpdate() && !createTaskValueObject.DeletedAt.IsSetToNull()`（`deletedAt` 缺省/空串 ⇒ `IsSetToNull` ⇒ 真复活，仍应 `+1`）。当前不可达 ⇒ 仅加固建议，不阻断本单。
    - 检查项/评论的覆盖分支无可迁移外键（无 `projectId`/`parentTaskId`）⇒ 无同类问题，不改。
...
| `infrastructure/persistence/task/repoImpl.go`                                                                        | 计数调整专用方法（直写列 + bump `updated_at`，**不经 UpdateTask**）；相关方法改 `DBFrom(ctx, repo.db)` 以加入外层事务；**`Upsert` 覆盖分支带出旧 `project_id`/`parent_task_id` 或变更标志（B12）** |
| `infrastructure/persistence/project/repoImpl.go`                                                                     | `task_count` 调整/批量重算方法（级联场景写最终值）                                                                                                                                                 |
| `application/task/appImpl.go`                                                                                        | 写路径包 `txManager.Do` + 发布事件（E1/E3/E4/E5/E6）；`needReadWrite` 扩展 ProjectId/ParentTaskId（B4）；**`CreateTask` 的 upsert 覆盖分支发 E5/E6（B12）**                                        |
