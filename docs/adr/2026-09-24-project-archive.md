...
| **提示唯一来源（`T190`/`T191`）** | **规则**：`ARCHIVED_READONLY` 的**唯一用户可见提示**来自**守卫**（`archive.readOnlyHint`，**本地化**）；**通知层对该码静默** —— **不得**透出**原始错误码**（`ARCHIVED_READONLY`），也**不得**再提示一次。**错误码本身仍须返回调用方**（机器判定/测试/调用方分支用；同 `write-gate` 同型）⇒ 本规则**只约束展示、不约束返回值**。**落点**：优先**通知层入口一处**（`presentation/task/utils/error-message.ts` 的 `translateTaskError`：`errString === ARCHIVED_READONLY_ERROR` ⇒ 视为已提示，**不产出文案**）；备选 handler 侧短路。**依据**：`T190` 行为级探针（拦一次**弹两条**：守卫本地化 + 原始错误码）+ **PM 裁定收敛为单条** + 实现 **`T191`** |
...
> **提示唯一来源（规则理由）**：**「同一拒绝 = 一条用户可见提示」** 为不变式。**离线闸门是同型事件的先行案例**：`OFFLINE_READONLY` 全仓**无任何调用方把该码送入提示层**（仅 `write-gate.ts` 定义处出现）⇒ 天然单条。本次等于把「离线闸门的**隐含约定显式化**」，因此二者**不得出现实现分叉**：**守卫负责提示（本地化）+ 通知层对闸门码静默**。另：`translateTaskError` 的设计回退是「**非领域错误原样透出**」，而 `ARCHIVED_READONLY` 是**表现层控制流码**（非领域错误）⇒ 该回退对它**语义不适用**，故必须在通知层识别并静默（而非把码登记为领域错误码）。
