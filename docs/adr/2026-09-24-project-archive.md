...
- **日期**：2026-09-24
- **状态**：⏳ **有条件可行**（**无「不可行」项**；**阻塞项 1 项** = `DEF-36` 前置修复；**DP-1…DP-5 待 PM 拍板**）｜⛔ **纯设计，未改任何代码**（含服务端）
...
| **Q5** | **`DEF-12`**       | **②（`sort` 无白名单 = 注入面）同批修**；**①（`isArchived=false` 空操作）同批修但只修显式 `false` 分支**（**不改服务端默认**，避免改变移动端可见结果）。**「移动端将看到归档任务」单独登记 + DP-1**。跨仓 ⇒ 须 PM 批准                                                                                                                                                                  |
...
| 子项  | 事实                                                                            | **裁定**                                                                                                                                                                                      |
| :---- | :------------------------------------------------------------------------------ | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **①** | `ByTaskArchived(false)` ⇒ `return db`（**空操作**）；`repoImpl.go:287` 直接透传 | **同批修**（`false` ⇒ `Where("archived_at IS NULL")`），**但仅修显式 `false` 分支**，**不改「未传 `isArchived`」的默认**（改默认会改变**移动端可见结果** ⇒ 触「移动端零改动」的**知情边界**） |
| **②** | `query.Sort` 无白名单：`Order(ToSnakeCase(field) + " " + direction)`            | **同批修（安全项，必做）**：字段白名单 + `direction ∈ {asc,desc}`；与归档正交但同属 `DEF-12`                                                                                                  |

**理由**：① 的「修 `false` 分支」是**纯语义修正**（谓词按字面工作），**当前无生产调用方**（客户端 local-first + 移动端不传 `isArchived`）⇒ **零现网影响**；而「改默认」会**改变移动端可见结果**（用户可见）⇒ 属**产品口径**，**不得**由技术裁定单方决定 ⇒ **DP-1**。② 是**注入面**，无理由拖延。

**跨仓纪律**：`nao-todo-server` 改动须 PM 批准（本 ADR 为**只读**引用）；服务端须补契约测试（`false` ⇒ 排除归档；`sort` 非法字段/方向 ⇒ 拒绝或忽略并留证）。

**登记（不随本批）**：**归档上线后移动端会看到归档任务**（移动端不过滤 + 服务端默认不排除）⇒ **DP-1**（三选一：a 移动端补过滤〔违红线，需用户授权〕/ b 服务端列表默认排除归档〔改默认，须核实 `/sync/pull` 不受影响 —— 已核实 **pull 不经过 `ByTaskArchived`**，仅 `repoImpl.go:287` 使用 ⇒ **技术上安全**〕/ c 接受并登记）。
...
| R-8  | 移动端将看到归档任务（服务端默认不排除 + 移动端不过滤）                | 可见性不一致（P1）         | **DP-1**（PM/用户拍板）                                                    |
...
| #        | 议题                                                                                                                                                                                          | 建议                                                                                            |
| :------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------- |
| **DP-1** | **移动端可见归档任务**（R-8）：a 移动端补过滤（违「移动端零改动」红线，需用户授权）/ b **服务端列表默认排除归档**（**已核实 `/sync/pull` 不经 `ByTaskArchived`** ⇒ 技术上安全）/ c 接受并登记 | **b**（一次性对齐两端；须用户/PM 知情「移动端将不再显示归档任务」）                             |
...
- **阻塞项：1 项** —— **`DEF-36` 前置修复**（P0；未修即接线 = 误删数据）。**其余无阻塞**：DP-1…DP-5 不影响 P0/P1 开工（DP-1 只影响 P2 的服务端项）。
- **登记（非阻塞）**：R-3（大清单推送量）· R-4（push 表序契约）· R-5（收集箱口径缺口，**非本批引入**）· R-6（复位非像素级）· R-8（移动端可见归档，DP-1）· R-9（文案承诺）· R-10（级联后 base 陈旧窗口）。
