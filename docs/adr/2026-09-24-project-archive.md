...
| **通知入口覆盖清单（`T189` 复核，逐文件核代码）** | **已覆盖 = 14 点 / 8 文件**（含 `isArchivedReadOnlyError` 短路）：`task/utils/error-message.ts:68` · `task/handlers/task.ts:43` · `task/components/dialogs/creator/use-creator.ts:84` · `task/components/multi-select/use-batch-executor.ts:87`（批量汇总剔除）· `task/components/task-details/use-task-view-object.ts:146,214,232` · `project/handlers/project.ts:153,177,220` · `web/calendar/monthly/use-calendar-schedule.ts:77,146,175` · `web/calendar/monthly/use-calendar-monthly.ts:207`。**经枚举得知未覆盖 = 3 点（均 `P2`；不改变拦截正确性与返回码，仅违反本行「单条提示」不变式；均**可达**）：① `web/src/components/tasks/project/archive-project-action.ts:48-50` —— 在已归档清单视图上再点「归档」⇒ `archive` 被拦，仍弹 `dialog.projectArchiveFailed` + **原始码**；② `presentation/project/components/dialogs/updater/use-project-updater.ts:63` —— 在归档清单上点「修改清单名称和描述」⇒ `update` 被拦，仍弹 `'清单更新失败：' + 原始码`；③ `presentation/task/components/dialogs/reminder/use-task-reminder.ts:65` —— 已归档任务到点提醒 ⇒「延时提醒」`snooze` 被拦，仍弹**原始码**。三者**修法同型**：入口加 `isArchivedReadOnlyError` 短路（各约 1 行）⇒ **待 PM 裁定：本批修 or 登记缺陷**（缺陷池编号由 PM 给）。                       |
