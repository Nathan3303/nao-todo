...
| I2  | `列宽 = k × 容器宽 ÷ 列数 ≥ 25px`（参考容器 1200px） | 档位**永不比现状更窄**                                                  |
...
| ×4   | 4   | 10min | 144  | 33.3      | 44.4      | 3.333         | `HH:MM`  | 48     | 粒度细化  |
...
- 改三级：**60min**（`--cal-border` 实线）> **30min**（当前 55% 混色）> **sub-30**（更低混色，**仅当 `columnMinutes < 30` 时启用**）。
...
- **登记（不阻塞）**：×4 档下 30 分钟 = 100px，吸附落定跳变明显。若用户人眼验收反馈「放大后落点仍太粗」，那属**吸附量子自身的决定**（30→15），须**另立单**并同步 D4 / `MIN_SPAN_MIN`，**不得**与缩放耦合。
...
| C2  | `columnMinutes ∈ {30,15,10}` 且 `30 % columnMinutes === 0`；`MIN_SPAN_MIN = 30` 为**分钟常量、不随档位变**（10min 档自动为 3 列最小宽）                |
...
| C6  | 禁 `列数 × N` 背景 DOM；列头节点数 = 列数（×4 = 144）；格线仍 CSS 渐变                                                                                 |
...
| 10  | `nowLeft`                                                                | **零改动** ✓ 同理                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 11  | 缩放三入口（**契约已定，供 T78-B / T79 / T80 直接断言**）                | ① **按钮**：`.day-header` 右组新增 2 个，`data-testid="day-zoom-out"` / `"day-zoom-in"`，`aria-label="缩小时间轴"` / `"放大时间轴"`，`theme="icon,ghost"`，到端 `disabled`；**不设**独立复位按钮（复位 = `$mod+0`）。② **快捷键**（**keys 字面量已定死**）：`useShortcut` 三条，id = `calendar.dayzoom.in` / `calendar.dayzoom.out` / `calendar.dayzoom.reset`，keys 分别 = **`$mod+=`** / **`$mod+-`** / **`$mod+0`**，在 day view 内注册、`scope: CALENDAR_KEY_SCOPE`，随 day view 卸载自动注销。**依据**：`packages/shared/commands/matcher.ts` 的 `parseKeys` 以 `+` 作分隔符 ⇒ **禁止**书写 `$mod++`；`matchesKeyEvent` 对未声明修饰键做**严格检查**（`!concrete.has('shift') && event.shiftKey ⇒ false`）⇒ `$mod+=` 只在**无 shift** 的 `Ctrl/⌘ + =` 命中，US 布局下 `Ctrl+Shift+=`（产生 `+`）**不命中**（有意如此，避免与主绑定语义重叠）。③ **滚轮**：`.day-body` 上 `@wheel.ctrl.prevent`；**方向**：`deltaY < 0` → **放大一档**，`deltaY > 0` → **缩小一档**，`deltaY === 0` → 忽略；需 `preventDefault` 阻止浏览器页面缩放。④ **不作断言项**：`theme="icon,ghost"` 属视觉规格（无稳定 DOM 可断），归 T82 人眼验收。 |
