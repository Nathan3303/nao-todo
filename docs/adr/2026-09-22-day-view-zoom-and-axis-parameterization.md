...
- 规则函数：`label = minutes % 30 ? '' : (columnMinutes === 30 ? HH : HH:MM)`。
...
- **P1（采纳）**：抽 `export const DAY_SNAP_MINUTES = 30`（`apps/web/src/components/calendar/snap.ts`）为唯一量子常量；3 处调用改用它，并与 `MIN_SPAN_MIN` / `MIN_TASK_MINUTES` 归一；新增不变量断言 `DAY_SNAP_MINUTES === MIN_SPAN_MIN === MIN_TASK_MINUTES === 30`（把「缩放不改吸附」从口头约定变为**可断言不变量**）。
...
| 10  | `nowLeft`                                                                | **零改动** ✓ 同理                                                                                                                                                                                                                   |
| 11  | 缩放三入口                                                               | 头部按钮（`.day-header` 右组，`theme="icon,ghost"`，到端 `disabled`）+ 3 条 `useShortcut`（在 day view 内注册，`scope: CALENDAR_KEY_SCOPE`，随 day view 卸载自动注销）+ `@wheel.ctrl.prevent`（挂 `.day-body`）                     |
