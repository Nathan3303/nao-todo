...
| 10  | `nowLeft`                                                                | **零改动** ✓ 同理                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 11  | 缩放三入口（**契约已定，供 T78-B / T79 / T80 直接断言**）                | ① **按钮**：`.day-header` 右组新增 2 个，`data-testid="day-zoom-out"` / `"day-zoom-in"`，`aria-label="缩小时间轴"` / `"放大时间轴"`，`theme="icon,ghost"`，到端 `disabled`；**不设**独立复位按钮（复位 = `Ctrl/⌘ 0`）。② **快捷键**：`useShortcut` 三条，id = `calendar.dayzoom.in` / `calendar.dayzoom.out` / `calendar.dayzoom.reset`（沿用 `calendar.nav.*` 命名范式），键位 `Ctrl/⌘ +` / `Ctrl/⌘ -` / `Ctrl/⌘ 0`，在 day view 内注册、`scope: CALENDAR_KEY_SCOPE`，随 day view 卸载自动注销。③ **滚轮**：`.day-body` 上 `@wheel.ctrl.prevent`（`Ctrl/⌘ + 滚轮`；需 `preventDefault` 阻止浏览器页面缩放） |
...
- **r1 内勘误（2026-09-22，QA T78 报备后 PM/arch 收口）**：① D1.1 标签公式**自相矛盾已修正**（见 §3-D1.1「公式勘误」）；② D2 常量落法由「三重等式」改为「单一来源 + 两常量断言」（见 §3-D2）；③ §5.3 #11 补齐**入口 DOM / 快捷键 / 滚轮契约**（原缺，新增）。
