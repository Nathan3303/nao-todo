...
- **日期**：2026-09-23
- **状态**：✅ **有条件通过（r2 修订，2026-09-23）** —— **DP-1 已批准**（C-59 作用域收窄为「业务数据面」+ 偏好/设置面显式例外）· **DP-2 作废**（用户裁定：web 也可离线改偏好）· **DP-3 / DP-4 / DP-6 已裁定** · **DP-5 = 后续单**｜⛔ **纯设计，未改任何代码**
...
**冲突语义（LWW 是否仍成立）**：✅ **仍成立**，且**不因 web 离线写而调整**：

- 判据**只用服务端时间**（设置面 = `user_configs.updated_at`；普通清单偏好 = 服务端行 `updated_at`），客户端本地时间戳**仅用于 UI 与队列合并顺序**，**不作冲突判据**（PS-8 不变）。
- 同步前**先比对**：`server.updatedAt` vs 本地持久化的「上次同步到的服务端版本」`SETTINGS_SYNCED_AT`（/ 按行 `syncedUpdatedAt`）：
    - 相等 ⇒ 远端未变，本地脏则**推送**；
    - **远端更新** ⇒ **远端胜**（应用远端、清本地脏）—— 即「**离线期间他端已改 ⇒ 本端离线改动被丢弃**」（低频、影响小；见 R-2/R-10，v1 显式接受）；
    - 远端更旧（时钟回拨）⇒ 视为未变，**推送**本地。
- **离线改动不带可信时间戳**，故**不能**用「我改得晚」来主张覆盖 ⇒ 保持 r1 的保守 LWW（服务端权威），**不引入客户端时钟**。
...
3. **冲突语义已按终态选型**：**LWW + 服务端时间为权威**（与「与后端解决更新冲突」一致）；阶段二若升级为**字段级 `updatedAt` 合并**，属**additive 增强**（blob 内加字段 + 按单位持久化 `syncedUpdatedAt`），**不推翻**本设计。
...
- **统一判据 = LWW，服务端时间为权威**（**客户端时间戳不作判据**，PS-8）。**两端本地优先不改变本判据**。
...
- **普通清单偏好**：**按行 LWW**（读时 / 推送前对账：**服务端行 `updated_at` 更新 ⇒ 应用远端**；否则本地脏 ⇒ 回传）。
- **已知局限（显式接受）**：
    1. 设置面为**整快照覆盖** ⇒ 两台设备并发改**不同**偏好时，后推者会覆盖前者（丢一台的改动）。
    2. **离线期间他端已改** ⇒ 本端离线改动在联网后按「远端胜」被丢弃（见 §D-1b / R-10）。
    - v1 **接受**（设置低频、影响小、可再改）；**升级路径** = blob 内每偏好带 `updatedAt` 做字段级合并 + 按单位持久化 `syncedUpdatedAt`（不在本批）。
...
- **R-10 离线期间他端已改 ⇒ 本端离线偏好改动被 LWW 丢弃**（影响：低 —— 偏好低频、可再改）｜应对：显式接受（§D-1b / §D-3）；升级路径 = 字段级 `updatedAt` 合并；QA 覆盖「离线改 + 他端在线改」最坏情形。
...
| M3   | 客户端普通清单偏好：**两端**「本地写 + 队列回传 + 读时对账」（web 由远端直连**改本地仓储**）       | rd-fe | M0（DP-1）      |
...
- **待办（非阻塞）**：**DP-5（tagPreference）后续单**（R-9 / R-14）；**§10 C-59 措辞由 PM 落地**（用户过目后）。
