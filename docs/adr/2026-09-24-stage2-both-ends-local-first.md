...
- **日期**：2026-09-24
- **状态**：⏳ **有条件可行**（**DP-1…DP-4 已拍板**；**r3（2026-09-24，T161）：2B 定向设计已落 §9**（OCC 契约 / 冲突 UX / 多标签 push / 偏好收口 / 迁移优化裁定；DP-2B-1…5 待拍板））｜⛔ **纯设计，未改任何代码**（含服务端）
...
- **推荐 (i)**：与 2A 三处 `meta` 纯追加（`mirror-status`/`preference-queue`/`conflict-journal`）**同一先例**（刻意避开 version bump）。
...
**9.2.4 R-15（上限张力）**：`CONFLICT_JOURNAL_LIMIT = 50` 环形淘汰 ⇒ **丢最旧败方快照** ⇒ 与「不丢数据」有张力。缓解（**推荐**）：**提高上限**（如 200）+ **淘汰时可见提示**（「更早冲突已折叠」），**不新增存储机制**；**不做**无限保留。
...
- **临界区内含**：网络往返 + **journal 写入**（`appendConflict`）⇒ 与 pull 侧 journal 写**互斥** ⇒ 无并发写（**必须**：journal 的 `meta` 单记录为 RMW）。
...
- **客户端改动面**：`projectPreferences` 记录加 `syncedServerUpdatedAt`（非索引，同 §9.1.6-(i)）；**读时对账升级** = 「本地缺失 **或** 服务端 `updatedAt > syncedServerUpdatedAt` ⇒ 拉取并应用（**远端胜 + journal**）」；**推送前对账/带 base**：不匹配 ⇒ 远端胜（不覆盖）+ journal；匹配 ⇒ 推送 + 落新版本。
...
| :----- | :---------------------------------------------------------------------------------------------------------------------------------------------- | :------------ | :---- | :-------------------------------------------------------------------- |
| **W0** | **前置**：C-44 协议评审（`baseUpdatedAt` + `stale` 枚举值 + 偏好回传）· **DP-2B-1/2/3 拍板** · **R-2 消费决策**                                 | arch / PM     | —     | 评审结论 + DP 裁定入档（可并入本 ADR r4）                             |
...
> **依赖硬约束**：**W1 → W2 → W3**（`stale` 必须先能产生再能展示）；**W2 → W4-偏好**（复用同一版本字段/比较/ journal）；**W0 未完成不得开工 W1**（C-44 协议评审为硬前置）。
...
| S17 | `docs/adr/2026-09-23-web-offline-local-first-and-security-posture.md`（C-44 邻接）                                            | **arch**  | 加「2B OCC 为 additive、经 C-44 协议评审」指针（**待 PM 批准后落**）                           | ⏳ 后续 |
...
| #           | 议题                                                                                | 建议                                                                          |
| :---------- | :---------------------------------------------------------------------------------- | :---------------------------------------------------------------------------- |
| **DP-2B-1** | **base 存储**：业务行非索引字段 (i) vs 新 Dexie 表 (ii)                             | **(i)**（不触 C-44 version bump；需 7 域本地写路径保留该字段）                |
| **DP-2B-2** | **OCC 是否全局启用**（业务 7 域全部）vs 仅新写                                      | **全局启用**（base 缺失即 LWW 回退 ⇒ 平滑）；接受 R-16 行为变更（需发布说明） |
| **DP-2B-3** | **`Outcome` 表达**：新增 `stale` 值 vs 复用 `conflict` + `reason` 字段              | **新增 `stale`**（可分流；additive）                                          |
| **DP-2B-4** | **R-2 消费范围**：是否同批消费 `error`（**2A 既有窄窗：服务端失败但客户端已出队**） | **同批消费**（否则 R-18 数据丢失风险；闭合 2A 登记）                          |
| **DP-2B-5** | **journal 上限**：保持 50 vs 提升（如 200）+ 折叠提示                               | **提升 + 折叠提示**（R-15）                                                   |
