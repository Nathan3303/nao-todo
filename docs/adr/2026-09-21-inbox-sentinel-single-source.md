...
| **r2** | **2026-09-25** | **加 scope 注记（不重写结论）**：文中「服务端已把 `''`/`'inbox'` 都视作收件箱」**仅 update / list 路径成立**，**create / `sync-push` 不成立**（字面则报错），且 **pull 出参 = `FormatID(ProjectId)` = `userId`**。依据 = **`T187` 实测**（`docs/qa/2026-09-25-r5-inbox-roundtrip-probe.md`，基线 `b7bb3305`）+ `T175c`；详见 `docs/adr/2026-09-24-project-archive.md` §16 |
...
> **※ 口径 scope（2026-09-25 补，`T175d`）**：上表「**服务端**」项指 **update（`converters.go:128-133`）/ list（`:201-206`）路径的入参表示**；**create / `sync-push` 路径不成立**（字面量 `'inbox'` ⇒ `idutil.ParseID` 报错），且 **pull 出参为 `FormatID(ProjectId)` = `userId`**（非 `'inbox'`）。依据 = **`T187` 实测**（`docs/qa/2026-09-25-r5-inbox-roundtrip-probe.md`）；**本行结论（数据面 `'inbox'`）不变**；详见 `docs/adr/2026-09-24-project-archive.md` §16。
...
> **※ scope 注记（2026-09-25 补，`T175d`，不改变上述否决结论）**：该条理由中「**服务端已把 `''`/`'inbox'` 都视作收件箱**」的适用范围为 **update（`converters.go:128-133`）/ list（`:201-206`）**；**create / `sync-push` 路径不成立**（字面量 `'inbox'` ⇒ `idutil.ParseID` 报错）。**否定结论不变**（“全局 VO 边界归一”仍被否决；但理由应升级为「各域在**自己的边界**归一，不外溢」——与 `2026-09-24-project-archive.md` §16 的修法 **C+B** 同思想）。依据 = `T187` 实测。
...
> **2026-09-24（`T175` 互记，清单归档）**：归档的「**单任务脱归档 ⇒ 归属收集箱**」**必须写 `'inbox'`（内建 id），禁写 `''`** —— 数据面单一真源 = `'inbox'`，`''` 仅搜索内部/URL token；服务端虽对二者等价归一为 `userId`，但**客户端本地仓储按字面 `'inbox'` 过滤** ⇒ 写 `''` 会当场「任务消失」。见 `docs/adr/2026-09-24-project-archive.md` **§5**（及 §1.3-C2/C3）。
